...
mod stable_toposort_normal {
    ...
    #[test]
    fn empty() {
        let o = stable_toposort::<&str>(vec![], []).unwrap();
        assert!(o.is_empty());
    }

    #[test]
    fn single() {
        assert_eq!(stable_toposort(["a"], []).unwrap(), ["a"]);
    }

    #[test]
    fn two_independent() {
        assert_eq!(stable_toposort(["a", "b"], []).unwrap(), ["a", "b"]);
    }

    #[test]
    fn two_ordered() {
        assert_eq!(
            stable_toposort(["a", "b"], [("a", "b")]).unwrap(),
            ["a", "b"]
        );
    }
    ...
    fn chain_three() {
        assert_eq!(
            stable_toposort(["a", "b", "c"], [("a", "b"), ("b", "c")]).unwrap(),
            ["a", "b", "c"]
        );
    }
    ...
    #[test]
    fn diamond_stable() {
        let o = stable_toposort(["a", "b", "c"], [("a", "c"), ("b", "c")]).unwrap();
        assert_eq!(o[0], "a");
        assert_eq!(o[1], "b");
        assert_eq!(o[2], "c");
    }
    ...
    fn diamond_reverse_order() {
        let o = stable_toposort(["b", "a", "c"], [("a", "c"), ("b", "c")]).unwrap();
        assert_eq!(o, ["b", "a", "c"]);
    }

    #[test]
    fn cycle_three() {
        let r = stable_toposort(["a", "b", "c"], [("a", "b"), ("b", "c"), ("c", "a")]);
    ...
    fn five_node_chain() {
        ...
        assert_eq!(stable_toposort(nodes, edges).unwrap(), ["v1", "v2", "v3", "v4", "v5"]);
    }
    ...
    fn double_diamond() {
        ...
        let o = stable_toposort(nodes, edges).unwrap();
    ...
    fn cycle_contains_all_in_cycle() {
        let r = stable_toposort(["a", "b", "c", "d"], [("a", "b"), ("b", "c"), ("c", "d"), ("d", "a")]);
    ...
    fn edges_skip_unknown_nodes() {
        let o = stable_toposort(["a", "b"], [("a", "b"), ("a", "z"), ("z", "q")]).unwrap();
        assert_eq!(o, ["a", "b"]);
    }
    ...
    fn many_independent() {
        ...
        let o = stable_toposort(nodes.iter().map(String::as_str), []).unwrap();
        assert_eq!(o.len(), 20);
...
mod stable_toposort_by_key_normal {
    use super::*;

    #[test]
    fn empty() {
        let o = stable_toposort_by_key::<&str, i32>(vec![], [], |_| 0).unwrap();
        assert!(o.is_empty());
    }

    #[test]
    fn single() {
        assert_eq!(stable_toposort_by_key(["a"], [], |n| n.len()).unwrap(), ["a"]);
    }
    ...
    fn key_identity_alphabetic() {
        let nodes = ["c", "a", "b"];
        let edges = [("a", "c"), ("b", "c")];
        let o = stable_toposort_by_key(nodes, edges, |n| *n).unwrap();
        assert_eq!(o, ["a", "b", "c"]);
    }
    ...
    fn key_length() {
        ...
        let o = stable_toposort_by_key(nodes, edges, |n| n.len()).unwrap();
        assert_eq!(o[0], "b");
        let pos = |n: &str| o.iter().position(|&x| x == n).unwrap();
    ...
    fn key_reverse_ord() {
        let nodes = [3, 1, 2];
        let edges = [(1, 2), (2, 3)];
        let o = stable_toposort_by_key(nodes, edges, |&n| std::cmp::Reverse(n)).unwrap();
        assert_eq!(o.len(), 3);
    ...
    fn key_tuple_primary_secondary() {
        ...
        let o = stable_toposort_by_key(nodes, edges, |n| (n.chars().next().unwrap(), n.chars().nth(1).unwrap()));
        let o = o.unwrap();
        let pos = |n: &str| o.iter().position(|&x| x == n).unwrap();
    ...
    fn by_key_cycle_error() {
        let r = stable_toposort_by_key(["x", "y"], [("x", "y"), ("y", "x")], |n| *n);
        assert!(r.is_err());
    }
    ...
    fn key_constant_all_same() {
        let nodes = ["a", "b", "c"];
        let edges = [("a", "c"), ("b", "c")];
        let o = stable_toposort_by_key(nodes, edges, |_| 0).unwrap();
        assert_eq!(o.len(), 3);
    ...
    fn key_negated_ordering() {
        ...
        let o = stable_toposort_by_key(nodes, edges, |n| -(n.len() as i32)).unwrap();
        let pos = |n: &str| o.iter().position(|&x| x == n).unwrap();
    ...
    fn key_mod_3_then_value() {
        ...
        let o = stable_toposort_by_key(nodes, edges, |&n| (n % 3, n)).unwrap();
    ...
    fn by_key_preserves_topo() {
        ...
        let o = stable_toposort_by_key(nodes, edges, |n| n.as_bytes()[0]).unwrap();
        let pos = |n: &str| o.iter().position(|&x| x == n).unwrap();
    ...
    fn key_bool_false_first() {
        ...
        let o = stable_toposort_by_key(nodes, edges, |&(b, _)| b).unwrap();
        assert_eq!(o[0].0, false);
        assert_eq!(o[1].0, true);
    }
}

mod stable_toposort_by_key_extra {
    use super::*;
    ...
    fn key_first_char() {
        ...
        let o = stable_toposort_by_key(nodes, edges, |n| n.chars().next().unwrap()).unwrap();
        let pos = |n: &str| o.iter().position(|&x| x == n).unwrap();
    ...
    fn key_ord_chain_reverse_key() {
        let nodes = [3, 2, 1];
        let edges = [(1, 2), (2, 3)];
        let o = stable_toposort_by_key(nodes, edges, |&n| 4 - n).unwrap();
        assert_eq!(o, [1, 2, 3]);
    }
    ...
    fn key_three_way_merge() {
        ...
        let o = stable_toposort_by_key(nodes, edges, |n| n.parse::<i32>().unwrap()).unwrap();
        assert_eq!(o, ["1", "2", "3", "4"]);
    }

    #[test]
    fn key_cycle_four() {
        let r = stable_toposort_by_key(
            ["a", "b", "c", "d"],
            [("a", "b"), ("b", "c"), ("c", "d"), ("d", "a")],
            |n| *n,
        );
    ...
    fn key_disconnected_groups() {
        ...
        let o = stable_toposort_by_key(nodes, edges, |n| n.chars().nth(1).unwrap()).unwrap();
        assert_eq!(o.len(), 4);
        let pos = |n: &str| o.iter().position(|&x| x == n).unwrap();
    ...
    fn key_complex_graph() {
        ...
        let o = stable_toposort_by_key(nodes, edges, |&n| n % 5).unwrap();
    ...
    fn key_borrow() {
        ...
        let o = stable_toposort_by_key(nodes, edges, |n| n.as_bytes()).unwrap();
    ...
    fn key_ord_float_like() {
        let nodes = [1, 2, 3];
        let edges = [(1, 2), (2, 3)];
        let o = stable_toposort_by_key(nodes, edges, |&n| (n as f64).to_bits()).unwrap();
        assert_eq!(o, [1, 2, 3]);
    }
